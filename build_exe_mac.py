"""
Build script for packaging WorkAttendanceApp Streamlit application into macOS .app bundle.
...
try:
    ...
    print()
    print("2. Make sure ALL files are copied, especially the _internal folder")
    print()
    print("="*60)
    print()
    
    # Try to show a message box on macOS
    try:
        import subprocess
        error_msg = f"WorkAttendanceApp Error:\\n\\n{type(e).__name__}: {str(e)}"
        subprocess.run(['osascript', '-e', f'display dialog "{error_msg}" with title "WorkAttendanceApp Error"'])
    except:
        pass
...
def build_pyinstaller_command(files):
    ...
    cmd = [
        ...
        "--windowed",  # Hide console window (creates .app bundle)
        "--clean",  # Clean cache before building
        "--noconfirm",  # Overwrite without asking
        "--additional-hooks-dir=.",  # Use custom hooks from current directory
        "--add-data=run_app.py:.",  # Add launcher (note: colon separator for macOS)
    ]
    
    # Add each file with proper path separator for macOS (colon instead of semicolon)
    for file in files:
        file_path = Path(file)
        if file_path.exists():
            ...
            cmd.append(f"--add-data={src_file}:{dest_dir}")  # Colon separator for macOS
...
def run_build(cmd):
    ...
    try:
        ...
        print("   2. Double-click: WorkAttendanceApp.app")
        ...
        print("   - You may need to allow the app in Security & Privacy settings")
        print()
        ...
        print("✓ Debug version created: WorkAttendanceApp_DEBUG")
        print("   Use this version to see error messages if the app doesn't start")
        print()
...
def main():
    ...
    print()
    print("╔" + "=" * 58 + "╗")
    print("║" + " " * 10 + "Streamlit macOS App Builder" + " " * 23 + "║")
    print("║" + " " * 5 + "Packaging WorkAttendanceApp for macOS" + " " * 20 + "║")
    print("╚" + "=" * 58 + "╝")
    print()
